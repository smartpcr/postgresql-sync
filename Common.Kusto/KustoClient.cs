...
namespace Common.Kusto
{
    ...
    public class KustoClient : IKustoClient
    {
        ...
        public async Task SwapTable(string targetTable, string backupTable)
        {
            var tempName = DateTime.Now.ToString("yyyyMMddHHmm");
            var renameTargetToTemp = CslCommandGenerator.GenerateTableRenameCommand(targetTable, tempName);
            await adminClient.ExecuteControlCommandAsync(kustoSettings.DbName, renameTargetToTemp);

            var renameBackupToTarget = CslCommandGenerator.GenerateTableRenameCommand(backupTable, targetTable);
            await adminClient.ExecuteControlCommandAsync(kustoSettings.DbName, renameBackupToTarget);

            var renameTempToBackup = CslCommandGenerator.GenerateTableRenameCommand(tempName, backupTable);
            await adminClient.ExecuteControlCommandAsync(kustoSettings.DbName, renameTempToBackup);
        ...
        private async Task<(List<T> updates, List<T> inserts)> CheckExistingRecords<T>(string tableName, List<T> items, string idPropName)
        {
            ...
            while (true)
            {
                ...
                logger.LogInformation($"id query: {idQuery}");
                var reader = await queryClient.ExecuteQueryAsync(
                    kustoSettings.DbName,
                    idQuery,
                    GetClientRequestProps());
