...
| summarize LastWriteTime=max(IngestionTime)";
        ...
        public async Task<int> Count(string countQuery, string countFieldName, CancellationToken cancel)
        {
            var count = await kustoClient.ExecuteScalar<Int64>(countQuery, countFieldName, cancel);
            return (int) count;
        }
    }
}